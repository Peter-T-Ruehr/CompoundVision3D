...
<!-- | -->
<!-- <center> ![](../man/figures/logo.png){width=1.8in} </center> -->
...
```{r define_folders, include=FALSE, warning=FALSE, message=FALSE}
...
```{r imported_eye_3D_plot, echo=FALSE, fig.cap="A caption", out.width = '50%'}
knitr::include_graphics("./imgs/imported_eye_3D_plot.png")
```
...
```{r search_diameter_definition, echo=FALSE, fig.cap="A caption", out.width = '50%'}
knitr::include_graphics("./imgs/search_diameter_definition.png")
```
...
##### Calculate distances of vertices from local plane
...
```{r SEM_colors_both, echo=FALSE, fig.cap="3D Plot of *Drosophila *eye with raw local heights (**left**) and log-transformed, quantile-filtered local heights (**right**).", out.width = '80%'}
knitr::include_graphics("./imgs/SEM_colors_both.png")
```
...
```{r thresholds, echo=FALSE, fig.cap="A caption", out.width = '80%'}
knitr::include_graphics("./imgs/thresholds.png")
```
...
```{r SEM_colors_thresholds, echo=FALSE, fig.cap="A caption", out.width = '50%'}
knitr::include_graphics("./imgs/SEM_colors_thresholds.png")
```
...
```{r facets_auto_process, echo=FALSE, fig.cap="A caption", out.width = '80%'}
knitr::include_graphics("./imgs/facets_auto_process.png")
```
...
```{r facets_auto_final, echo=FALSE, fig.cap="A caption", out.width = '50%'}
knitr::include_graphics("./imgs/facets_auto_final.png")
```
...
```{r facet_positions_corrected, echo=FALSE, fig.cap="A caption", out.width = '50%'}
knitr::include_graphics("./imgs/facet_positions_corrected.png")
```


#### Bring all data into the same coordinate system
##### Import landmarks
Now we will import landmarks created in 3D Slicer to use for the rotation of the eye. You may use any landmark data at your own convenience. The resulting tibble should have the columns `LM`, `x`, `y`, and `z` though. 
...
```{r facet_positions_corrected_w_LMs, echo=FALSE, fig.cap="A caption", out.width = '70%'}
knitr::include_graphics("./imgs/facet_positions_corrected_w_LMs.png")
```

##### Load log file from cropping
Now we will load the log file that was created during the cropping process to get the pixel sizes of the head ROI and the eye ROI. If you did not use the cropping script, you may skip loading a crop log file and instead enter the respective pixel sizes manually. If the head and the eye ROI have the same pixel size, you will still need to assign this value to both pixel sizes, the one of the head ROI and the one of the eye ROI.

```{r import_crop_log_file, eval=FALSE, warning=FALSE, message=FALSE}
# define log file with full path
crop_log_file_location <- "./Drosophila/crop_log/Drosophila_crop.log")[1]

# load crop log file
crop_log_data <- read_delim(crop_log_file_location,
                            delim = " = ",
                            col_names = FALSE,
                            show_col_types = FALSE) %>% 
  rename(var = X1,
...
The `crop_log_data` tibble of this example on *Drosophila* can also be loaded with
```{r read_crop_log_file, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
crop_log_data <- CompoundVision3D::crop_log_data

# show landmarks
print(crop_log_data)
``` 

xxx: Add info that we are using eye 2 with early image of head and its eyes

##### Extract pixel sizes from the different ROIs
###### Head ROI
```{r get_px_size_head_ROI, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
# head ROI pixel size
...
###### Eye ROI
```{r get_px_size_eyes_ROI, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
# original/eye ROI pixel size
...
##### Extract ROI coordinates from the different ROIs
xxx: add figure to illustrate what the ROI coordinates mean
```{r extract_ROI_coords, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
# define ROI names
...
curr_ROI_coordinates <- get_ROI_coordinates(ROIs,
                                            crop_log_data)
print(curr_ROI_coordinates)
```

##### Tranlsate the eye surface and facet coordinates according to the ROI coordinates
```{r add_ROI_coords, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
##### Plot if all coordinates in the same coordinate system
Now we will plot the eye surface, facet position and landmark coordinates to see if they are in the same coordinate system. Note that the coordinate system is not yet rotated, so that the left eye appears right in the figure, and the dorsal head landmarks on the lower side of the figure.

```{r plot_translated_coordinates, eval=FALSE, warning=FALSE, message=FALSE}
# plot eye in SEM colors
plot3d(local_heights_translated %>% 
         select(x,y,z), 
       col = local_heights_translated$local_height_col_log, 
       size= 5,
       aspect = "iso")
...
```{r facet_positions_translated_w_LMs, echo=FALSE, fig.cap="A caption", out.width = '70%'}
knitr::include_graphics("./imgs/facet_positions_translated_w_LMs.png")
```

##### Combine all data for further analyses

```{r combine_facets, eval=TRUE, warning=FALSE, message=FALSE}
...
#### Rotate data according to global coordinate system
